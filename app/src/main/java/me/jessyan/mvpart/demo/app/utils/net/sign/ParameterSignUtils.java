package me.jessyan.mvpart.demo.app.utils.net.sign;
...
//        if (url.contains(ApiConfiguration.Domain.CARD_BASE_URL)) {
            ...
            String token = "666";
            sMap.put("token", TextUtils.isEmpty(token) ? "token" : token); //授权token,登录后需要传token，默认值：token
...
//        Timber.tag("Request_Url").w(url.replace("\n", ""));
        LogUtils.e("Request_Url",url.replace("\n", ""));
    }
